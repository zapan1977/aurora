...
Tableau_USER=""
Tableau_PWD=""
Tableau_TSM="/opt/tableau/tableau_server/packages/customer-bin.20181.18.0404.1605/tsm"

# Tableua Login
${Tableau_TSM} login --username ${Tableau_USER} --password ${Tableau_PWD}

# Tableua 서버 토폴로지 및 구성 데이터 백업
${Tableau_TSM} settings export -f ${BACKUP_DIR}/pf_tableau_settings_${BACKUP_DATE}.json

# Tableua 리포지토리 데이터 백업
${Tableau_TSM} maintenance backup -f pf_tableau_repo_${BACKUP_DATE}.tsbak

# Tableua session logout
${Tableau_TSM} logout --username ${Tableau_USER} --password ${Tableau_PWD}
